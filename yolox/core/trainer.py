...
class Trainer:
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            # print(loss_meter.items(0))
            if self.rank == 0:
                for k, v in loss_meter.items() :
                    self.tblogger.add_scalar("train/{}".format(k) , v.latest, self.epoch + 1)
                self.tblogger.add_scalar("train/{}".format("lr"), self.meter["lr"].latest, self.epoch + 1)
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                ...
                # self.tblogger.add_scalar("total_loss" , self.epoch + 1)
