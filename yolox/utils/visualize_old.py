...
def vis(img, boxes, scores, z, cls_ids, conf=0.5, class_names=None ):
    ...
    for i in range(len(boxes)):
        ...
        # one_line = (str(x0), str(y0), str(x1), str(y1), class_name, str(float(score)), str(az))     # txt資料內容
        one_line = (class_name, str(az))    # class, z
        ...
        cv2.putText(img, text_z, (int((x0+x1)/2-(txt_z_size[1]/2)), int((y0+y1)/2)), font, 1, (0,0,255) , thickness=3)
