...
def make_parser():
    ...
    parser.add_argument("-c", "--ckpt", default="YOLOX_outputs/yolox_x_newz2/best_ckpt.pth", type=str, help="ckpt for eval")
    parser.add_argument(
        "--device",
        default="gpu",
        type=str,
...
def add_z(img, gt_labels, predict, match_num, current, total, image_name, name) :
    ...
    if ( miss_num <= 1 ) :
        ...
        for i in range(len(gt_list)) : 
            ...
            # print("------------------ :.{}".format(predict[match][4]))
            gt_labels[i].append(str(abs(predict[match][4]))+ "\n")
            # print("------------------ :.{}".format(abs(predict[match][4])))

        #新建一個txt
        
        with open('datasets/newbdd2_1/{}.txt'.format(name), 'w') as f:
        ...
        shutil.copyfile(image_name, 'datasets/newbdd2_1/{}.jpg'.format(name))
