...
def make_parser():
    ...
    parser.add_argument(
        "-f",
        "--exp_file",
        default="exps/example/custom/yolox_x_newz2.py",
        type=str,
...
def add_z(img, gt_labels, predict, match_num, current, total, image_name, name) :
    ...
    if ( len(predict) > 0 ) :
        ...
        #新建一個txt
        if (len(newgt_labels)> 0) :
            with open('datasets/white_bdd/labels/{}.txt'.format(name), 'w') as f:
                for i in range(len(newgt_labels)) :
                    ...
                    for j in range(len(newgt_labels[i])) :
                        ...
                        if( j != 5 ) :
                            f.write(" ")
            cv2.imwrite('datasets/white_bdd/images/{}.jpg'.format(name), img)
